-- ============================================================================
-- AU vs AI — 0009 Round 1 becomes FOUR images, and timing adjustments
--
-- Four images divides 500 evenly (125 each) instead of the awkward
-- 167/167/166 split, and gives a fairer read on whether a student can actually
-- spot AI rather than getting lucky on three.
--
-- New play time:  4x8s + 20s + 8s = 60s exactly, before reveals.
-- ============================================================================

-- Widen the slot range. Existing three-slot attempts remain valid.
alter table public.attempt_round1
  drop constraint if exists attempt_round1_slot_range;

alter table public.attempt_round1
  add constraint attempt_round1_slot_range check (slot between 1 and 4);

-- Timings (§10). All still editable from /admin without a deploy.
update public.event_settings
set round1_ms_per_image = 8000,
    round2_draw_ms      = 20000,
    round3_ms           = 8000
where id = 1;

-- ---------------------------------------------------------------------------
-- start_attempt — now freezes FOUR Round 1 images.
--
-- Still guarantees at least one real and one AI image, then fills the
-- remaining two from anywhere in the bank, then shuffles so slot order carries
